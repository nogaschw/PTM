...
class Config:
    def __init__(self):
        ...
        self.main_path = "OriginalData/codeworkout/MainTable.csv"
        self.code_path = "OriginalData/codeworkout/LinkTables/CodeStates.csv"
...
def run(config, main_df):
    ...
    main_df.to_csv("labeled_paths_all.tsv", sep="\t", header=True)
    
        
if __name__ == "__main__":
    config = Config()
    code_df = pd.read_csv(config.code_path)
    main_df = pd.read_csv(config.main_path)
